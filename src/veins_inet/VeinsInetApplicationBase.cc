...
VeinsInetApplicationBase::~VeinsInetApplicationBase()
{
    system("chmod +x transferOutput.sh");
    system("transferOutput.sh");
}
...
void VeinsInetApplicationBase::speedPayload(inet::Ptr<inet::Chunk> payload)
{

    payload->removeTagIfPresent<CreationTimeTag>(b(0), b(-1));
    auto creationTimeTag = payload->addTag<CreationTimeTag>();

    //creationTimeTag->setCreationTime(traciVehicle->getSpeed());

    //int tempSpeed = 45 + simTime();
    creationTimeTag->setCreationTime(-45+simTime());
